...
if __name__ == '__main__':
    ...
    try:
        ...
        dataset_id = upload_dataset(client, pth_to_resources + "/iris.csv",
                                    'iris', pid)
        ...
        possible_warning_error = client.change_feature_types_check(
            dataset_id, 'iris_cat', changes)
        ...
        dataset_id2 = upload_dataset(client,
                                     pth_to_resources + "/iris_no_trgt.csv",
                                     'iris_test', pid, False)
        for best_model_key, best_conf in result['models'].items():
            ...
            possible_warning_error = client.predict_outcome_check(
                analysis_id, dataset_id2, best_model_key)
            for key, messages in possible_warning_error.items():
                print(key + ": " + str(messages))
            ...
            prediction_id = client.predict_outcome(analysis_id, dataset_id2,
                                                   best_model_key)
            ...
            print("prediction result head:\n\t" + str(
                client.get_prediction_result(prediction_id).split('\n')[0:5]))
        ...
        print("Remaining datasets in {}: ".format(pid) +
              str(client.get_datasets(pid)))
