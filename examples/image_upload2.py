...
def upload_type2(jadbio_client: jadbio.client.JadbioClient,
                 project: int,
                 name: str,
                 data_folder: str,
    ...
    for target_class in class_names:
        image_files = image_files_in_dir(
            os.path.join(data_folder, target_class))

        for image_file in image_files:
            sample = image_file.split('.')[0]
            class_dict[sample] = target_class
            sample_path_dict[sample] = os.path.join(data_folder, target_class,
                                                    image_file)
    ...
    try:
        ...
        task = jadbio_client.image_upload_init(project, name, target_path,
                                               True, description)
...
d = upload_type2(jadbio_client=client,
                 project=project,
                 name='image',
